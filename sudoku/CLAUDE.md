...
- `POST /api/stats` upserts the user's profile (via RLS INSERT policy), then inserts a new solve time record. The profile upsert ensures the `profiles` FK constraint is satisfied even if the `handle_new_user` trigger didn't fire.
